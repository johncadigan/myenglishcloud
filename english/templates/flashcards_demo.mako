...
if(level=='Flashcard1'||level=='Flashcard2'||level=='Flashcard3'||level=='Flashcard4'||level=='Flashcard5'||level=='Flashcard6'||level=='Flashcard7'||level=='Flashcard8'){
	    ...
	    setupCard : function(flashcardnum, count, cardType) {
			...
			var flashcardHTML = $('<div class="flashcard row '+cardType+'" id="flashcard' + (count - 1) + '"></div>');

  			var flashcardmain = $('<div class="flashcardmain col-lg-9 col-md-9 col-sm-9 col-xs-9"></div>');

                        flashcardHTML.append('<div class="progress col-lg-12 col-md-12 col-sm-12 col-xs-12"><div class="bar" style="width:'+(100*cdVars['count']/flashcardCount)+'%;"></div></div><div class="span4"></div>');

                        var pictureHTML = '<div class="flashcard-picture col-lg-3 col-md-3 col-sm-3 col-xs-3"><img  src="'+flashcard.picture+'"></div>';
			...
			fcardinfoHTML.append('<div class="row sentence"><h2>'+newSentence+'</h2></div>');
			}
			else{
			fcardinfoHTML.append('<div class="row sentence"><h2>'+flashcard.sentence+'</h2></div>');
			}
			...
			if(cardType=='Flashcard'){
                        var input = '<input class="row input" id="' + inputName + '" name="' + inputName
                                    + '" type="text"/><br>';
			...
			return '<div class="flashcard row '+cardType+'" id="flashcard' + (count - 1) + '">'+flashcardHTML.html()+'</div>';
	    ...
	    setupBPanel: function(distractorArray, orderArray, answer){
			...
			for (i in distractorArray){
				response = '<button id="'+flashcard.position+'"class="btn btn-default btn-lg col-lg-5 col-md-5 col-sm-5 col-xs-5 incorrectButton">' + distractorArray[i] + '</button>';
				selectionArray[i] = response;
			   }
selectionArray[distractorArray.length] ='<button id="'+flashcard.position+'"class="btn btn-default btn-lg col-lg-5 col-md-5 col-sm-5 col-xs-5 correctButton">'+answer+'</button>';			
			...
			for (i in RorderArray){
				...
				c = RorderArray[indx];
				d = RorderArray[num];;
				buttonLayer=$('<span class="col-lg-12 col-md-12 col-sm-12 col-xs-12 drow"><span><br>');
		...
		return panelHTML;
			...
			if(timeout == false){
			plugin.method.demoMessage('Since you ran out of time, this card is wrong.')
			timeout = true;
			}		
			}
            ...
            rotateElement: function(element,angle)
	{
		...
		$('#saveAndquit').show()
		plugin.method.demoMessage('Flashcards are introduced a few at a time, making it easier to learn.');
		});
		...
		$(pauseButton).hide();
		plugin.method.demoMessage('The pause button hides the card to prevent cheating.');	
		...
		plugin.method.demoMessage('The clock starts ticking again...');
	  ...
	  moveUp: function(ctype){
	   ...
	   clickIncorrect : function(IncorrectButton) {
		...
		plugin.method.demoMessage('Since you answered incorrectly, this card will be the same next time.')
	    },
		...
		plugin.method.demoMessage('Since you answered correctly, this card will be harder next time.')
	    },
		...
		correct = 'True'
		plugin.method.demoMessage('Since you answered perfectly, this card will give you more points next time.')                
		}
		...
		plugin.method.demoMessage('The closer you are the more points you get.')
		}
		...
		plugin.method.demoMessage('Since were close, you get some points.')
                }
		...
		if(nxtFlashcard.hasClass('Show')){
		plugin.method.demoMessage('Flashcards are introduced a few at a time, making it easier to learn.');
		}
		else if(nxtFlashcard.hasClass('4Source')||nxtFlashcard.hasClass('8Source')){
		plugin.method.demoMessage('Cards start out easy and get harder.')
		}
		else if(nxtFlashcard.hasClass('4Target')||nxtFlashcard.hasClass('8Target')){
		plugin.method.demoMessage("Cards start out in your language and then switch into English.")
		}
	...
	$(targets.demoMessage).fadeOut('slow').hide().html(message).fadeIn('slow');
...
<h2><span id='demo-message' class='orange'>Flashcard Demo</span></h2>


%if flashcard_json == '':
<div class='col-lg-3 col-md-3 col-sm-3 col-xs-3'>
<%include file="demo_vocab_box.mako" args="vocab_items=vocabulary, error=vb_error"/>
</div>
%endif


%if vocabulary == '' and flashcard_json == '':
<form id='language' method=GET>
Which language do you speak?
<select id="preferred_language" name="preferred_language">
%for language_option in language_options:
<option value="${language_option['id']}">${language_option['e_name']}-${language_option['n_name']}</option>
%endfor
...
%elif flashcard_json=='':
<span class='lead'>If you are logged in, you can find a box to add flashcards on <strong>Lesson</strong> and <strong>Reading</strong> pages.</span>
<br><br><i class="icon-arrow-left icon-3x"></i>
<br><br>
<span class='lead'>If there are ${language} translations for the words, the most popular translations will appear. <br><br>If there are no translations in ${language}, you will have to enter them.</span>
...
<div id="fcardDrill">
        <div class="drillArea">
            <div class="drillHeader">
                ...
                <div class='centerDiv'><a id="start_button" class="invisible btn btn-default btn-lg text-center" href="${request.route_url('my_flashcards')}">No flashcards!</a></div>
            </div>
...
<div id="point-results" class='invisible'>
<form id="point-form" method=POST action=${request.route_url('register')}>
<input type="hidden" name="activity_type" value='drill'>
<input type="hidden" name="activity_id" value=${drill}>
